...
priority_queue<pair<float,int> > priq[10];
    ...
    for(int i = 1; i <= num; i ++)
    {
        ...
        for(int j = 0; j< berth_num;j++)
        {
            if(g[berth[j].x][berth[j].y] == gg){
                min_d = min(min_d , d2[j][x][y]);
            }
        }
        priq[gg].push({(float)val/min_d,goods_cnt[gg]});
    }
    for(int i = 0; i < robot_num; i ++)
    {
        int sts;
