...
int Input()
{
    ofstream log("push.txt", std::ios_base::app); 
    scanf("%d%d", &id, &money);
    int num;
    scanf("%d", &num);
    for(int i = 1; i <= num; i ++)
...
{   
    ...
    for(int zhen = 1; zhen <= 15000; zhen ++)
    {
        ...
        for(int i = 0; i < boat_num; i ++){
            ...
            if(boat[i].status == 0 || boat[i].status == 2)
            {
            ...
            else if(boat[i].status == 1 && boat[i]. == 2*i){
                boat[i].cnt ++;
                ...
                else if(boat[i].cnt >150) //&& boat[i].num < (boat_capacity/2))
                {
                    printf("ship %d %d\n",i,i*2+1);
                    boat[i].cnt = 0;
                    log<<" ship2";
            ...
            else if(boat[i].status == 1 && boat[i].pos == 2*i+1){
                boat[i].cnt ++;
                int cnt = min(berth[boat[i].pos].val, min(berth[boat[i].pos].loading_speed, boat_capacity - boat[i].num));
                boat[i].num  += cnt;
                berth[boat[i].pos].val -=cnt;
