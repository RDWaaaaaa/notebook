...
void change(int &s, int &t, int type) {
    if (type == 0) {
        ...
        for(int i = 0; i < 4; i++){
            ...
            if (xx < 0 || xx >= n || yy < 0 || yy >= n) continue;
            if (ch[xx][yy] == '*' || ch[xx][yy] == '#') continue;
            if((xx != robot[t].x || yy !=robot[t].y)&&isempty(s,xx,yy)){
                printf("move %d %d\n",s,i);
                robo_move[s][0]=-1;
            }
        }
...
{   
    ...
    for(int zhen = 1; zhen <= 15000; zhen ++)
    {
            ...
            if(robot[i].target_id == -1){
                ...
                for(int j = 0; j < 4; j ++ ){
                    ...
                    if(d2[robot[i].target_id][xx][yy] < d2[robot[i].target_id][robot[i].x][robot[i].y]){
                        ...
                        log<<" st= "<<st<<" ";
                        log<<"now"<<robot[i].x<<","<<robot[i].y<<";next"<<xx<<","<<yy;
                        if(st == robot_num + 1){
                            //printf("move %d %d\n",i,j);
                            robo_move[i][0] = j;
        ...
        for(int i = 0; i < robot_num;i++)
        {
            log<<i<<" "<<robot_pos[i].now_x<<" "<<robot_pos[i].now_y<<" "<<robot_pos[i].ne_x<<" "<<robot_pos[i].ne_y<<endl;
            if(robo_move[i][0]<=3 && robo_move[i][0]>=0){
                printf("move %d %d\n",i,robo_move[i][0]);
                // if(robot[i].goods){
                //     if(robot_pos[i].ne_x == gorobot[i].target_id && )
                // }
