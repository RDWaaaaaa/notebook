    ...
    for(int zhen = 1; zhen <= 15000; zhen ++)
    {
        ...
        for(int i = 0; i < boat_num; i ++){
            ...
            if(boat[i].status == 0 || boat[i].status == 2)
            {
                ...
                int cnt = min(berth[boat[i].pos].val, min(berth[boat[i].pos].loading_speed, boat_capacity - boat[i].num));
                // log<<"check "<<berth[boat[i].pos].val<<" "<<berth[boat[i].pos].loading_speed<<" "<<boat_capacity - boat[i].num<<endl;
                boat[i].num  += cnt;
                
                berth[boat[i].pos].val -=cnt;

                
                if(15000 - id == berth[boat[i].pos].transport_time + 1|| boat[i].num == boat_capacity)
                {
                    printf("go %d\n",i);
                    log<<"num="<<boat[i].num;
                    if(boat[i].num == boat_capacity){
                        log<<"case1 "<<boat_capacity<<endl;
                    }
                    else{
                        log<<"case2"<<endl;
                    }
                ...
                else if(boat[i].cnt>150)
                {
                    log<<"num2="<<boat[i].num<<endl;
                    if(boat[i].num<=boat_capacity/2){
                        if(15000 - id >= berth[boat_idx].transport_time + 1 + 500){
                            printf("ship %d %d\n", i, boat_idx);
